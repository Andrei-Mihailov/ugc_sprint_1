...
from api.v1.schemas.auth import TokenSchema
...
async def get_authorize_url(provider: str):
    ...
    if oauth_service is None:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="Unknown provider"
        )
...
async def receive_verification_code(
    ...
    if not login_result:
        raise HTTPException(status_code=HTTPStatus.BAD_GATEWAY, detail="Can't login")
    return login_result
